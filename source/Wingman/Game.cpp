...
void Game::initUI()
{
	...
	this->levelText.setColor(Color::Magenta);
	this->levelText.setPosition(10, 4);

	this->expText.setFont(font);
	this->expText.setCharacterSize(14);
	this->expText.setColor(Color::White);
	this->expText.setPosition(215, 8);

	this->healthText.setFont(font);
	this->healthText.setCharacterSize(14);
	this->healthText.setColor(Color::White);
	this->healthText.setPosition(100, 8);
	...
	//bars init
	this->playerMaxHealthBar.setSize(Vector2f(90, 14));
	this->playerMaxHealthBar.setFillColor(Color(125, 125, 125, 100));
	this->playerMaxHealthBar.setPosition(75, 10);

	this->playerHealthBar.setSize(Vector2f(90, 14));
	this->playerHealthBar.setFillColor(Color::Red);
	this->playerHealthBar.setPosition(75, 10);

	this->playerMaxExpBar.setSize(Vector2f(90, 14));
	this->playerMaxExpBar.setFillColor(Color(125, 125, 125, 100));
	this->playerMaxExpBar.setPosition(175, 10);

	this->playerExpBar.setSize(Vector2f(90, 14));
	this->playerExpBar.setFillColor(Color(255, 255, 0, 150));
	this->playerExpBar.setPosition(175, 10);

	this->playerPowerupBar.setSize(Vector2f(90, 10));
	this->playerPowerupBar.setFillColor(Color::Blue);
	//Enemy text init 
	...
	this->scoreText.setPosition(10, 30);
	...
	this->controlsText.setString(
		"DI CHUYEN: LEFT RIGHT TOP DOWN \nSPACE: SHOOT\nRALT: SHIELD\nTAB: Stats window\nP: PAUSE/CONTROLS (START GAME)\nESC: QUIT!"
	);
	this->controlsText.setPosition(50, 400);

	//PlayerStatText init
	this->playerStatsText.setFont(font);
	this->playerStatsText.setFillColor(Color::White);
	this->playerStatsText.setCharacterSize(15);
...
void Game::initialize()
{
	...
	//Set up sound
	if (!this->soundPickupB.loadFromFile("Sound/soundPickup.wav")){}
	else
		this->soundPickup.setBuffer(soundPickupB);

	if (!this->soundEnemyDieB.loadFromFile("Sound/soundEnemyDie.wav")) {}
	else
		this->soundEnemyDie.setBuffer(soundEnemyDieB);

	if (!this->soundEnemyBeDamagedB.loadFromFile("Sound/soundEnemyBeDamaged.wav")) {}
	else
		this->soundEnemyBeDamaged.setBuffer(soundEnemyBeDamagedB);

	if (!this->soundPlayerBeDamagedB.loadFromFile("Sound/soundPlayerBeDamaged.wav")) {}
	else
		this->soundPlayerBeDamaged.setBuffer(soundPlayerBeDamagedB);

	if (!bgm.openFromFile("Sound/Stage.ogg")) {}
	else
	{
		bgm.play();
		bgm.setVolume(30.0f);
...
void Game::restartUpdate()
{
	...
	if (Keyboard::isKeyPressed(Keyboard::F1) || Keyboard::isKeyPressed(Keyboard::P))
	{
		bgm.play(); //reset bgm
...
void Game::updateUIPlayer(int index)
{
	...
	{
		// TEXT	
		this->levelText.setString("Level:" + std::to_string(this->players[index].getLevel()));
		this->healthText.setString(this->players[index].getHpAsString());
		this->expText.setString(this->players[index].getExpAsString());
		...
		this->playerPowerupBar.setPosition(this->players[index].getPosition().x , 
		...
		if (this->players[index].playerShowStatsIsPressed())
		{
			...
			this->playerStatsTextBack.setPosition(
				this->players[index].getPosition().x - 100.f,
	...
	for (size_t i = 0; i < this->enemies.size() && !enemyRemoved; i++)
	{
		...
		for (size_t k = 0; k < this->players.size() && !playerKilled; k++)
		{
			if (this->players[k].isAlive())
			{
				...
				{
					soundPlayerBeDamaged.play();

					int damage = this->enemies[i].getDamage()-this->players[k].getArmor();
...
void Game::drawUI()
{
	...
	{
		this->window->draw(this->playerMaxExpBar);
		this->window->draw(this->playerMaxHealthBar);
